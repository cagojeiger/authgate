...
| 영역 | 내용 |
|------|------|
| Authorization endpoint | `/oauth/authorize` 요청 검증, PKCE, redirect_uri 확인 |
...
| 영역 | 내용 |
|------|------|
| `op.Storage` 구현 | zitadel 인터페이스(필수 22개, Device 포함 시 24개)를 PostgreSQL로 연결 |
| upstream IdP 연동 | Google OAuth 코드 교환 → 유저 정보 취득 |
| 로그인 UI | 세션 확인, 약관 동의, auto-approve |
| 유저/세션 관리 | 유저 생성, 세션 CRUD, 계정 삭제 |
| auth request 완료 | 로그인 완료 후 auth request에 subject를 연결하고 완료 상태를 만들기 |
| refresh 상태 검증 | refresh_token으로 user를 찾아 `DeriveLoginState` 기준 차단 여부 판정 |
...
| Storage 인터페이스 | 필수 22개, Device Flow 포함 시 24개 |
...
### Negative
- zitadel이 HTTP 라우팅 소유 (`/.well-known/*`, `/oauth/*`)
- Storage 인터페이스 22개(+Device 2개) 메서드 학습 필요
