...
| Refresh Token 교환 | 토큰 회전 시점 판단, 새 토큰 발급 흐름 |

### authgate가 직접 처리하는 것

| 영역 | 내용 |
|------|------|
| `op.Storage` 구현 | zitadel 인터페이스(28개 메서드)를 PostgreSQL로 연결 |
...
| Storage 인터페이스 | 28개 메서드 (fosite 34개 대비 간결) |
