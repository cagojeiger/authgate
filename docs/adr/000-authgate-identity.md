...
**보존 기간은 각 데이터가 수행하는 기능에서 도출한다.** 기능이 끝나는 시점이 곧 보존 상한이며, 그보다 오래 갖고 있을 근거는 authgate에 없다. 만료된 세션·토큰은 인증에 쓸 수 없으므로 즉시, 폐기된 갱신 토큰은 재사용 조사 여유만큼, 패밀리 톰스톤은 소속 토큰이 모두 만료될 때까지 보관한다.

감사기록은 두 종류로 나눠 보존한다. **운영자 조치(`admin.*`)는 법령이 요구하는 접속기록**이므로 법정 하한 이상을 보관하고 그 아래로는 기동을 거부한다. **정보주체 활동(`auth.*`)은 그 의무의 대상이 아니므로** 침해조사에 필요한 기간만 식별 정보를 유지한 뒤 익명화한다. 성공한 토큰 갱신은 기록하지 않는다 — 자격증명의 마지막 사용 시각은 `refresh_tokens.used_at`이, 인증 사건은 `auth.login`이 이미 보유하며, 반복되는 정상 동작을 남기면 감사기록이 그것으로 채워진다.

법령 준수의 최종 책임은 authgate를 배포한 사업자에게 있다. authgate는 수단과 기본값을 제공할 뿐이며, 처리방침 공개·보호책임자 지정·유출 통지·취급자의 DB 직접 접근 통제는 authgate 밖에서 이루어져야 한다.
