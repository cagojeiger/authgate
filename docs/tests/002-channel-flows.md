...
| `browser-007` | `recoverable_browser_only`, 복구 후 auth_request 완료 상태 반영 실패 | Browser 재로그인 | 다음 재시도에서 정상 완료 | 복구 후 재시도 멱등성 |
...
| `mcp-002` | 미가입 | `/mcp/callback` | `signup_required` | MCP에서 신규 가입 불가 |
| `mcp-003` | `initial_onboarding_incomplete` | `/mcp/callback` | `signup_required` | MCP는 onboarding 채널이 아님 |
| `mcp-004` | `reconsent_required` | `/mcp/callback` | `signup_required` | MCP는 재동의 채널이 아님 |
| `mcp-005` | `recoverable_browser_only` | `/mcp/callback` | `account_inactive` | Browser 복구만 가능 |
| `mcp-006` | `inactive` | `/mcp/callback` | `account_inactive` | 차단 |
| `mcp-007` | auth code 발급 후 `code_verifier` 없이 토큰 교환 | `POST /oauth/token` | 토큰 발급 실패 | PKCE 강제 |
...
| `account-007` | `inactive` | `DELETE /account` | `account_inactive` | 비활성 계정 삭제 차단 |
