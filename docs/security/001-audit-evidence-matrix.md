...
| `auth.refresh_reuse_detected` | 폐기 refresh token 재사용 (family당 최초 1회) | user_id, IP, UA, created_at | `family_id` | `internal/storage/storage_auth_tokens.go` | `internal/storage/audit_test.go` | DONE |
