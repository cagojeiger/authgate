...
- **성공적으로 Device 토큰을 발급받으려면** `DeriveLoginState = onboarding_complete`여야 함 (Spec 001 경유, [ADR-000](../adr/000-authgate-identity.md) 정의)
- 사용자가 브라우저 접근 가능해야 함
...
| GET | `/device/auth/callback` | authgate 핸들러 | Device 전용 Google callback (state=user_code) |
...
sequenceDiagram
    ...
    alt 유효한 세션 있음
        ...
        AG->>AG: getSessionUser → DeriveLoginState + GuardLoginChannel 재검사
        ...
        AG->>U: 302 → Google 로그인 (state=user_code, redirect=/device/auth/callback)
        U->>G: Google 인증
        G->>U: 302 → /device/auth/callback?code=...&state=user_code
        U->>AG: /device/auth/callback → 기존 유저 확인
        AG->>AG: DeriveLoginState 검사
        Note over AG: onboarding_complete가 아니면 즉시 차단 (403)
        AG->>AG: 세션 생성
...
2. Google 로그인 redirect (state=user_code, redirect_uri=/device/auth/callback)
3. Google 인증 성공 → /device/auth/callback?code=...&state=user_code
4. 기존 유저 확인 → DeriveLoginState 검사 (onboarding_complete가 아니면 즉시 차단)
...
기존 유저를 찾더라도 `DeriveLoginState`가 `initial_onboarding_incomplete` 또는 `reconsent_required`이므로
`GuardLoginChannel(user, device)`에서 `signup_required`로 차단된다.
가입/재동의는 여기서 발생하지 않고, 브라우저 로그인 채널에서 먼저 완료해야 한다.
...
| 가입 미완료 사용자 | 브라우저 | `signup_required: please sign up via browser first` | 403 | 브라우저 가입 먼저 필요 |
| 비활성 계정 (disabled/deleted) | 브라우저 | `account_inactive` | 403 | 승인 불가 |
| pending_deletion 계정 | 브라우저 | `account_inactive` | 403 | 브라우저 복구만 가능 (GuardLoginChannel) |
...
채널 가드: `/device/auth/callback`(세션 생성 전)과 `approve` 두 시점 모두에서 [ADR-000 GuardLoginChannel](../adr/000-authgate-identity.md#guardloginchannel--공통-채널-가드)을 적용한다. `DeriveLoginState`가 `onboarding_complete`가 아니면 세션 생성/승인 불가 (403).
