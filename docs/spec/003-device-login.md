...
- **onboarding_complete** (Spec 001 경유, [ADR-000](../adr/000-authgate-identity.md) 정의: `status='active'` AND terms/privacy 동의 완료 AND 현재 버전 일치)
- 사용자가 브라우저 접근 가능해야 함
...
sequenceDiagram
    ...
    alt 유효한 세션 있음
        ...
        AG->>AG: getSessionUser → 유저 확인 (onboarding_complete)
        ...
        U->>AG: /device/auth/callback → 기존 유저 확인 → 세션 생성
...
4. 기존 유저 확인 → 세션 생성 → /device?user_code=XXXX로 302 redirect
5. 승인 페이지 표시 → 사용자 승인
```
...
채널 가드: 승인(approve) 시점에서 [ADR-000 GuardLoginChannel](../adr/000-authgate-identity.md#guardloginchannel--공통-채널-가드)을 적용한다. `onboarding_complete = false`이면 `signup_required` (403).
