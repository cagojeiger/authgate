...
- 모든 `/authorize` 요청은 `code_challenge` + `code_challenge_method=S256`를 반드시 포함해야 함. 예외는 `skip_pkce: true`를 명시한 confidential 클라이언트뿐이다.

## 클라이언트 유형

| 유형 | client_secret | PKCE | 예시 |
|------|-------------|------|------|
| **confidential** | 있음 (bcrypt 해시 저장) | 필수 (`skip_pkce: true`로 면제 가능) | 백엔드 웹 앱, BFF |
...
public 클라이언트에는 설정할 수 없다(로드 시 거부). 시크릿이 없는 클라이언트에서
PKCE는 인가 코드 가로채기에 대한 유일한 방어이기 때문이다.
...
- 모든 `/authorize` 요청에서 PKCE S256 필수 (plain 불허, code_challenge 누락 불허). `skip_pkce: true`인 confidential 클라이언트만 예외이며, 그 경우 client_secret이 보호 수단이 된다. 클라이언트 조회에 실패하면 PKCE를 강제한다(fail-safe).
