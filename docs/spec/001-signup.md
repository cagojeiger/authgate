...
| `email` | 표시용, 약관 증적 | 아니오 (IdP에서 변경 가능) | 아니오 |
| `name` | 표시용 | 아니오 | 아니오 |
...
**`terms_accepted_at IS NULL` 또는 `privacy_accepted_at IS NULL`인 `active` 계정 = `initial_onboarding_incomplete` ([ADR-000 DeriveLoginState](../adr/000-authgate-identity.md) 참조).**
`active`는 계정이 정지/삭제되지 않았다는 의미이지, 토큰 발급 가능을 뜻하지 않는다.
토큰 발급 조건: `DeriveLoginState = onboarding_complete` (동의 완료 + 현재 버전 일치).

### 가입 미완료 레코드 정리

사용자가 약관 페이지에서 이탈하면 `initial_onboarding_incomplete` 상태의 유저가 남는다.
...
-- 생성 후 7일 경과 + initial_onboarding_incomplete 유저 삭제
-- NOTE: reconsent_required (동의했으나 버전 변경) 유저는 대상이 아님
DELETE FROM users
WHERE status = 'active'
