...
```
SIGNUP_EMAIL_DOMAINS=example.com,corp.example.com
```

| 규칙 | 내용 |
|------|------|
| 매칭 | **정확히 일치**. `example.com`은 `sub.example.com`을 허용하지 않는다 (와일드카드는 운영자가 명시적으로 적어야 한다) |
| 표기 | `example.com` / `@example.com` 모두 허용. 대소문자 무시 |
| 검증 | 점이 없거나 `@`·공백이 섞인 항목은 **시작 거부** (오타가 조용히 전원 차단으로 이어지지 않게) |
