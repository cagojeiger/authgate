...
| 검증 | ASCII DNS 이름이 아닌 항목(URL, 끝 점, 빈 라벨, `_`, 유니코드 — 국제화 도메인은 `xn--` 형태로 적는다), 점이 없거나 `@`·공백이 섞인 항목, `*`가 선두 `*.` 이외의 위치에 있는 항목, `*.com`처럼 TLD를 대상으로 하는 와일드카드, 설정은 했지만 항목이 하나도 없는 값(`,`)은 **시작 거부** (오타가 조용히 전원 차단 또는 전원 허용으로 이어지지 않게). `*.co.uk` 같은 public suffix는 검사하지 않는다 |
...
| 이메일 도메인 | 주소의 도메인도 ASCII DNS 이름이어야 한다. 공백·빈 라벨이 섞였거나 유니코드 소문자 변환으로 허용 도메인과 같아지는 주소(예: 켈빈 기호 `K`)는 `email_malformed`로 거부 |
| 신뢰 근거 | 게이트는 IdP가 준 `email`과 `email_verified`만 믿는다. **조직 소속(Google Workspace `hd`)은 보지 않는다.** 누구나 회사 주소로 개인 Google 계정을 만들 수 있고 한 번 받은 `email_verified`는 유지되므로, 퇴사자나 공용 메일 별칭도 도메인이 맞으면 가입할 수 있다. 조직 관리 계정만 받으려면 `hd` 검사가 필요하다(현재 미지원) |
