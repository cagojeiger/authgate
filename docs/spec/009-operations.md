...
| `SIGNUP_EMAIL_DOMAINS` | X | — | 콤마 구분 도메인 목록 (예: `example.com,corp.example.com`). 비어 있으면 **가입 제한 없음**(기본). 설정되면 해당 도메인 이메일만 신규 가입 가능. 정확히 일치해야 하므로 `example.com`은 `sub.example.com`을 허용하지 않는다. `@example.com` 표기도 허용. 잘못된 항목(점 없음, `@` 포함 등)은 **시작 거부**. 활성화 시 `email_verified=false` 주소는 도메인이 맞아도 거부한다 — 미검증 주소는 도메인을 증명하지 못하므로. **가입만** 막으며 기존 계정 로그인은 영향 없음. 거부는 `auth.signup_denied`로 감사 기록 |
