...
authgate 입장에서는 같은 코드 경로를 탄다. `/login` 및 `/login/callback`에서 Spec 002의 `DeriveLoginState`와 `GuardLoginChannel`을 동일하게 적용한다. 따라서 `onboarding_complete`가 아닌 사용자는 MCP 토큰 발급 경로로 진입할 수 없다. 이 스펙은 MCP 특이사항만 다룬다.
