...
- 로그인 상태 판정은 [ADR-000 DeriveLoginState](../adr/000-authgate-identity.md)를 따른다
...
```mermaid
stateDiagram-v2
    [*] --> active: 계정 생성 (Spec 001, onboarding 미완료일 수 있음)
...
|------|-------------|-----------------|----------|------|
| **active** (`onboarding_complete`) | 허용 | 허용 | 허용 | 정상 |
| **active** (`reconsent_required`) | 허용 → 약관 재동의 | 차단 (`signup_required`) | 차단 | 버전 변경 |
| **active** (`initial_onboarding_incomplete`) | 허용 → 약관 표시 | 차단 (`signup_required`) | 차단 | 최초 미동의 |
...
**원자성 규칙**: 복구와 세션 생성은 단일 트랜잭션이다. 실패 시 전부 롤백되어 "복구됐는데 세션이 없는" 중간 상태가 생기지 않는다. CompleteAuthRequest가 트랜잭션 밖이라면, 복구는 유지되고 다음 로그인에서 즉시 정상 완료된다 (재시도 멱등).
...
- `user_identities` 3단계에서 명시적 삭제됨 → `GetUserByProviderIdentity` → `ErrNotFound`
...
```
로그인 → DeriveLoginState → reconsent_required → terms.html → 동의 → 진행
```

모든 기존 사용자가 다음 로그인 시 재동의해야 한다. 재동의 전까지 토큰 발급/갱신 불가.
`reconsent_required` 유저는 정상 유저이므로 cleanup 대상이 **아니다** ([ADR-000 DeriveLoginState](../adr/000-authgate-identity.md) 참조).

## Cleanup Lifecycle

authgate는 3종의 cleanup을 별도 lifecycle로 관리한다:

| 종류 | 대상 | 조건 | 주기 |
|------|------|------|------|
| **onboarding cleanup** | `initial_onboarding_incomplete` | `status='active'` AND `terms_accepted_at IS NULL OR privacy_accepted_at IS NULL` AND `created_at < NOW() - 7일` | Spec 001 참조 |
| **deletion cleanup** | `recoverable_browser_only` | `status='pending_deletion'` AND `deletion_scheduled_at < NOW()` | 이 스펙 3단계 |
| **token cleanup** | 만료/폐기된 토큰 | `expires_at < NOW()` 또는 `revoked_at IS NOT NULL` | Spec 005 참조 |

**주의**: `reconsent_required` 유저(`terms_accepted_at IS NOT NULL`, 버전만 불일치)는 어떤 cleanup에도 해당하지 않는다.
