...
- **onboarding_complete** = `status = 'active'` AND `terms_accepted_at IS NOT NULL` AND `privacy_accepted_at IS NOT NULL` AND `terms_version = 현재 TERMS_VERSION` AND `privacy_version = 현재 PRIVACY_VERSION` ([ADR-000](../adr/000-authgate-identity.md) 공식 정의)
