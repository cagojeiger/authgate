...
| 항목 | 설명 |
|------|------|
| **책임** | 로그인 조율 (upstream 호출 → user 조회/생성 → guard → 세션/토큰), Device 승인, 계정 삭제, cleanup |
...
| **의존** | 없음 |
| **사용처** | storage (session_id, token_hash 생성), service (device_code 발급) |
...
10. [service] → guard 결과에 따라:
      ...
      inactive → 403
11. [zitadel] /oauth/token → storage.AuthRequestByCode() → JWT 발급
```
