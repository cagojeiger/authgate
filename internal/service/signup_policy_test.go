...
func TestSignupDomainPolicy_MatchesExactDomain(t *testing.T) {
	...
	refused := []string{
		...
		"  user@Example.Com  ",  // whitespace is not part of any domain
		"user@example.com.",     // trailing dot
		"user@\u212Axample.com", // Kelvin sign would fold to k under Unicode lowercasing
...
// signup-domain-007: the email's domain must itself be a valid ASCII DNS name.
// A wildcard is a suffix test, so malformed domains that end in an admitted
// suffix, and Unicode look-alikes that lowercase into one, must not match.
func TestSignupDomainPolicy_WildcardRejectsMalformedDomains(t *testing.T) {
	p := signupDomainPolicy{domains: []string{".example.com", "korp.com"}}
	for _, email := range []string{
		"a@.example.com",
		"a@..example.com",
		"a@sub..example.com",
		...
		if ok, reason := p.allows(email, true); ok || reason != "email_malformed" {
			t.Errorf("%q: ok=%v reason=%q, want refused as email_malformed", email, ok, reason)
		}
	}
}

// signup-domain-008: a wildcard match is case-insensitive on the address and
// still requires a verified address.
func TestSignupDomainPolicy_WildcardCaseAndVerification(t *testing.T) {
	p := signupDomainPolicy{domains: []string{".example.com"}}
	if ok, reason := p.allows("User@Sub.EXAMPLE.com", true); !ok {
		t.Errorf("uppercase address under a wildcard was refused (%s)", reason)
	}
	if ok, reason := p.allows("user@sub.example.com", false); ok || reason != "email_unverified" {
		t.Errorf("unverified address under a wildcard: ok=%v reason=%q, want email_unverified", ok, reason)
	}
}
