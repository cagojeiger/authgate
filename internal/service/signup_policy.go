...
// valid one. The local part may itself contain '@' when quoted, so the split is
// on the last one. The domain must be an ASCII DNS name: lowercasing Unicode
// would let a look-alike such as the Kelvin sign fold into an admitted ASCII
// domain, and a malformed domain ("..example.com", trailing space) is not one
// any mailbox is at.
