...
type Config struct {
	...
	// "example.com" matches that domain exactly; "*.example.com" matches its
	// subdomains but not example.com itself (stored as ".example.com"). Only
	// signup is gated: accounts that already exist keep working regardless of
	// this list.
...
func Load() (*Config, error) {
	...
	// A value that is set but holds no entry (",", " , ") would otherwise
	// parse to an empty list, which means open signup: the opposite of what
	// an operator who set the variable meant.
	if strings.TrimSpace(os.Getenv("SIGNUP_EMAIL_DOMAINS")) != "" && len(c.SignupEmailDomains) == 0 {
		return nil, fmt.Errorf("SIGNUP_EMAIL_DOMAINS is set but lists no domain")
	}
...
func normalizeSignupDomains(raw []string) ([]string, error) {
	...
	for _, entry := range raw {
		...
		// Anything that is not an ASCII DNS name (a URL, a trailing dot, an
		// empty label, Unicode) can never equal an address's domain, so it
		// would refuse every signup without an error anywhere.
		if !domainname.Valid(base) {
			return nil, fmt.Errorf("%q is not a valid domain; use letters, digits and hyphens, and the xn-- form for internationalized domains", entry)
		}
