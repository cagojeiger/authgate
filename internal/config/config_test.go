...
func TestLoad_InvalidBoolEnvFails(t *testing.T) {
	for _, key := range []string{"ENABLE_MCP"} {
...
func TestLoad_SignupEmailDomains_RejectsMalformed(t *testing.T) {
	for name, value := range map[string]string{
		...
		"leading hyph":  "-example.com",
		"underscore":    "exa_mple.com",
		"unicode":       "例え.jp",
		"full-width":    "ＥＸＡＭＰＬＥ.com",
		"bad wildcard":  "*.exa_mple.com",
	} {
...
// config-signup-006: a value that is set but lists no domain is an error. It
// would otherwise parse to an empty list, which means open signup: the
// opposite of what setting the variable asked for.
func TestLoad_SignupEmailDomains_SetButEmptyIsRejected(t *testing.T) {
	for _, value := range []string{",", " , ", ",,,"} {
		t.Run(value, func(t *testing.T) {
			...
			if _, err := Load(); err == nil || !strings.Contains(err.Error(), "SIGNUP_EMAIL_DOMAINS") {
				t.Fatalf("Load with SIGNUP_EMAIL_DOMAINS=%q: err = %v, want a SIGNUP_EMAIL_DOMAINS error", value, err)
			}
		})
	}
}
