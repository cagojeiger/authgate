...
| `SIGNUP_EMAIL_DOMAINS` | – | Comma-separated domains allowed to create an account. Unset leaves signup open. `example.com` matches only that domain; `*.example.com` matches its subdomains but not the domain itself, so admitting both means listing both. Entries must be ASCII DNS names (use the `xn--` form for internationalized domains); malformed entries, or a value with no entries, stop startup. With it set, unverified addresses are refused too, and the gate trusts the IdP's `email`/`email_verified`, not Google Workspace membership (`hd`). Gates signup only — existing accounts keep working. |
