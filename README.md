...
| `SIGNUP_EMAIL_DOMAINS` | – | Comma-separated domains allowed to create an account (`example.com,corp.example.com`). Unset leaves signup open. Matches exactly, so `example.com` does not admit `sub.example.com`. With it set, unverified addresses are refused too. Gates signup only — existing accounts keep working. |
